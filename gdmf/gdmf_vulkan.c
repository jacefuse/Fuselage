...
static void gdmf_queue_lock(void)   { if (g_queueLockReady) { EnterCriticalSection(&g_queueLock); }

    return;
}
static void gdmf_queue_unlock(void) { if (g_queueLockReady) { LeaveCriticalSection(&g_queueLock); }
...
static void gdmf_upload_lock(void)   { if (g_uploadLockReady) { EnterCriticalSection(&g_uploadLock); }

    return;
}
static void gdmf_upload_unlock(void) { if (g_uploadLockReady) { LeaveCriticalSection(&g_uploadLock); }
...
static CRITICAL_SECTION g_deferLock;
...
void gdmf_vulkan_shutdown(void) {
    ...
    if (g_deferLockReady) {
        ...
        DeleteCriticalSection(&g_deferLock);
    }
...
static int gdmf_create_logical_device(void) {
    ...
    InitializeCriticalSection(&g_deferLock);
...
static void gdmf_defer_push(const GdmfDeferEntry* proto) {
    ...
    *e = *proto;
    EnterCriticalSection(&g_deferLock);
    e->framesLeft = g_swapchainImageCount + 1;
    e->next       = g_deferList;
    g_deferList   = e;
    LeaveCriticalSection(&g_deferLock);
