...
// Bumped on every mutation of Colors[] so consumers that mirror the table
// (GDMF's per-swapchain-image palette buffers) can skip re-packing and
// re-uploading 16KB every frame when nothing changed -- the common case.
// Starts at 1 so a zero-initialized "last generation seen" never matches
// before the first real upload. Mutations all happen either during single-
// threaded init or on the sim thread under the game-state lock, the same
// lock the consumer reads under -- no atomics needed.
static uint32_t g_colors_generation = 1;

uint32_t GetColorsGeneration(void) { return g_colors_generation; }
...
bool SetColorsPalette(unsigned char paletteIndex, Color* spritepalette) {
    if (paletteIndex >= 255) {
...
bool SetColorsPaletteColor(unsigned char paletteIndex, unsigned char colorIndex, Color color) {
    if (paletteIndex >= 255) {
        printf("Invalid palette index or palette size.\n");
        return false;
    }
...
Color GetColorsPaletteColor(unsigned char paletteIndex, unsigned char colorIndex) {
...
void InitColorsPalettes(void) {
    for (int i = 0; i < 255; i++) {
        for (int j = 0; j < 16; j++) {
            Colors[i][j] = GetColorsColorFromChar(i);
...
Color GetColorsColorFromChar(unsigned char input) {
    switch (input) {
...
Color GetColorsCommodoreColor(unsigned char input) {
    switch (input) {
...
Color GetColorsTandyColor(unsigned char input) {
    switch (input) {
...
Color GetColorsANSIColor(unsigned char input) {
    switch (input) {
